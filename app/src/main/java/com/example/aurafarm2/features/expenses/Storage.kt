...
suspend fun saveCategoryCorrection(context: Context, correction: CategoryCorrection) {
    context.settingsDataStore.edit { prefs ->
        ...
        val updated = current.filterNot { it.merchant.trim().lowercase() == normalized } +
            correction.copy(merchant = correction.merchant.trim(), category = correction.category)
        ...
        buildList {
            for (i in 0 until array.length()) {
                ...
                if (merchant.isNotBlank() && category.isNotBlank()) {
                    add(CategoryCorrection(merchant, category))
                }
            }
        }
    }.getOrElse { emptyList() }

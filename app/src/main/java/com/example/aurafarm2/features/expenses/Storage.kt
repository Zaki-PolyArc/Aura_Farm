...
val SMART_DETECTION_ENABLED_KEY = booleanPreferencesKey("smart_detection_enabled")
...
data class ExpenseEntry(
    ...
    val confidenceScore: Double = 100.0
)
...
fun categoryCorrectionsFlow(context: Context): Flow<List<CategoryCorrection>> =
    context.settingsDataStore.data.map { prefs ->
        decodeCategoryCorrections(prefs[CATEGORY_CORRECTIONS_KEY] ?: "[]")
    }

suspend fun saveCategoryCorrection(context: Context, correction: CategoryCorrection) {
    context.settingsDataStore.edit { prefs ->
        val current = decodeCategoryCorrections(prefs[CATEGORY_CORRECTIONS_KEY] ?: "[]")
        val normalized = correction.merchant.trim().lowercase()
        
        val existing = current.find { 
            it.merchant.trim().lowercase() == normalized && it.category == correction.category 
        }
        val newCount = (existing?.selectionCount ?: 0) + correction.selectionCount
        
        val updated = current.filterNot { 
            it.merchant.trim().lowercase() == normalized && it.category == correction.category 
        } + correction.copy(merchant = correction.merchant.trim(), selectionCount = newCount)
        
        prefs[CATEGORY_CORRECTIONS_KEY] = encodeCategoryCorrections(updated)
    }
}

suspend fun deleteCategoryCorrection(context: Context, merchant: String) {
    context.settingsDataStore.edit { prefs ->
        val normalized = merchant.trim().lowercase()
        val current = decodeCategoryCorrections(prefs[CATEGORY_CORRECTIONS_KEY] ?: "[]")
        prefs[CATEGORY_CORRECTIONS_KEY] = encodeCategoryCorrections(
            current.filterNot { it.merchant.trim().lowercase() == normalized }
        )
    }
}
...
fun encodeCategoryCorrections(corrections: List<CategoryCorrection>): String {
    val array = JSONArray()
    corrections.forEach { correction ->
        val obj = JSONObject()
        obj.put("merchant", correction.merchant)
        obj.put("category", correction.category)
        obj.put("selectionCount", correction.selectionCount)
        array.put(obj)
    }
    return array.toString()
}

fun decodeCategoryCorrections(raw: String): List<CategoryCorrection> =
    runCatching {
        ...
        buildList {
            for (i in 0 until array.length()) {
                ...
                if (merchant.isNotBlank() && category.isNotBlank()) {
                    add(CategoryCorrection(merchant, category, count))
                }
            }
        }
    }.getOrElse { emptyList() }
...
fun encodeExpenseEntries(entries: List<ExpenseEntry>): String {
    ...
    entries.forEach { entry ->
        ...
        obj.put("sourceApp", entry.sourceApp)
        obj.put("autoGenerated", entry.autoGenerated)
        obj.put("notificationHash", entry.notificationHash)
        obj.put("createdAtEpochMillis", entry.createdAtEpochMillis)
        obj.put("confidenceScore", entry.confidenceScore)
        array.put(obj)
    }
    return array.toString()
}
            ...
            for (i in 0 until array.length()) {
                ...
                add(
                    ExpenseEntry(
                        ...
                        notificationHash = obj.optString("notificationHash"),
                        createdAtEpochMillis = obj.optLong("createdAtEpochMillis", 0L),
                        confidenceScore = obj.optDouble("confidenceScore", 100.0)
                    )
                )
            }
        }
...
fun pendingTransactionsFlow(context: Context): Flow<List<PendingTransaction>> =
    context.settingsDataStore.data.map { prefs ->
        val raw = prefs[PENDING_TRANSACTIONS_KEY] ?: "[]"
        decodePendingTransactions(raw)
    }

suspend fun savePendingTransaction(context: Context, entry: PendingTransaction) {
    context.settingsDataStore.edit { prefs ->
        val current = decodePendingTransactions(prefs[PENDING_TRANSACTIONS_KEY] ?: "[]")
        val updated = current.filterNot { it.id == entry.id } + entry
        prefs[PENDING_TRANSACTIONS_KEY] = encodePendingTransactions(updated)
    }
}

suspend fun deletePendingTransaction(context: Context, entryId: String) {
    context.settingsDataStore.edit { prefs ->
        val current = decodePendingTransactions(prefs[PENDING_TRANSACTIONS_KEY] ?: "[]")
        prefs[PENDING_TRANSACTIONS_KEY] = encodePendingTransactions(current.filterNot { it.id == entryId })
    }
}

fun encodePendingTransactions(entries: List<PendingTransaction>): String {
    val array = JSONArray()
    entries.forEach { entry ->
        val obj = JSONObject()
        obj.put("id", entry.id)
        obj.put("amount", entry.amount)
        obj.put("merchant", entry.merchant)
        obj.put("type", entry.type)
        obj.put("dateEpochDay", entry.dateEpochDay)
        obj.put("sourceApp", entry.sourceApp)
        obj.put("notificationHash", entry.notificationHash)
        obj.put("createdAtEpochMillis", entry.createdAtEpochMillis)
        obj.put("suggestedCategory", entry.suggestedCategory)
        obj.put("confidenceScore", entry.confidenceScore)
        array.put(obj)
    }
    return array.toString()
...
fun decodePendingTransactions(raw: String): List<PendingTransaction> {
    return runCatching {
        val array = JSONArray(raw)
        buildList {
            for (i in 0 until array.length()) {
                ...
                add(
                    PendingTransaction(
                        ...
                        sourceApp = obj.optString("sourceApp"),
                        notificationHash = obj.optString("notificationHash"),
                        createdAtEpochMillis = obj.optLong("createdAtEpochMillis"),
                        suggestedCategory = obj.optString("suggestedCategory"),
                        confidenceScore = obj.optDouble("confidenceScore")
                    )
                )
            }
        }
